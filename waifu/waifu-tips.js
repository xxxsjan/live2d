...
function loadWidget(config) {
        ...
        ; (function welcomeMessage() {
            ...
            if (location.pathname === "/") {
                ...
                if (location.hostname === referrer.hostname)
                    ...
                    text = `Hello！来自 百度搜索 的朋友<br>你是搜索 <span>${referrer.search.split("&wd=")[1].split("&")[0]
                        }</span> 找到的我吗？`
                else if (domain === "so")
                    text = `Hello！来自 360搜索 的朋友<br>你是搜索 <span>${referrer.search.split("&q=")[1].split("&")[0]
                        }</span> 找到的我吗？`
    ...
    async function loadModel(modelId, modelTexturesId, message) {
        ...
        if (useCDN) {
            ...
            console.log(`${cdnPath}get/?id=${modelId}-${modelTexturesId}`)
            loadlive2d("cdnapi-live2d", `${cdnPath}model/${target}/index.json`)
        } else {
            loadlive2d("live2d", `${apiPath}get/?id=${modelId}-${modelTexturesId}`)
            console.log(`apiPath-Live2D 模型 ${modelId}-${modelTexturesId} 加载完成`)
        }
    }
    ...
    async function loadOtherModel() {
        ...
        if (useCDN) {
            if (!modelList) await loadModelList()
            const index = ++modelId >= modelList.models.length ? 1 : modelId
